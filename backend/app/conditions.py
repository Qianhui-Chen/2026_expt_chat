...
PROMPT_A_GENERIC = (
    "【本组：A·支持用户 × 通用脚本】\n"
    "【通用对话式衔接】第一段开头先自由生成一句不超过15个汉字的简短、模糊、宽泛的安慰或支持性套话，再直接衔接下方第一段脚本。"
    "衔接句只需提供一般性的安慰或支持，不得表现出已经理解用户的具体处境；"
    "不得假定用户表达了某一种特定内容，不得解释原因，不得提及任何具体事实。"
    "不得根据用户输入内容调整衔接句、脚本或建议，也不得回应用户具体的提问类型、事实或推理方向。"
    "在不改变本组立场、建议标题、数量、顺序及核心含义的前提下，允许适度调整非关键词句表达，以增强回应和对话的自然程度；但不得针对、回应、暗示、引用或转述用户提到的具体内容。"
...
PROMPT_B_GENERIC = (
    "【本组：B·反对用户 × 通用脚本】\n"
    "【通用对话式衔接】第一段开头先自由生成一句不超过15个汉字的简短、模糊、宽泛的思考型套话，再直接衔接下方第一段脚本。"
    "衔接句应保持客观冷静，并宽泛提示可能存在对立视角、其他解释或不同判断；"
...
GENERIC_ROUND_SCRIPTS: dict[str, tuple[tuple[str, tuple[tuple[str, str], ...]], ...]] = {
    "ingroup": (
        ...
        ("我会一直站在你这边。有些矛盾冲突的产生更多与他人的处理方式有关，不需要因为事情的结果过度责备自己。在进一步处理冲突时，保持相对平稳的情绪也有助于更好地判断和回应当前的问题。", (("保持情绪冷静", "沟通时尽量保持冷静，避免被一时的情绪左右。"), ("适当暂缓回应", "情绪激动时，可以暂时停止对话，给彼此一些缓冲时间。"), ("避免冲动回应", "在回应之前先思考，避免因一时冲动说出伤人的话。"))),
        ("我支持你，人际矛盾处理的过程中，首先需要看到自己的感受；在认可自己感受的同时，如果之后希望让双方的沟通更加顺利，也可以尝试一些减少彼此防御的沟通方式。比如，你可以试试：", (("适当肯定对方", "适当肯定对方在关系中做得好的地方。"), ("关注积极方面", "不要只关注冲突和问题，也注意关系中的积极部分。"), ("缓解防御心理", "通过积极的表达减少对方的防御感，促进共同解决问题。"))),
        ("我觉得这件事情里对方的表达确实存在一些问题。人际交往中遇到一些让自己不舒服的情况时，不必先怀疑自己。人际交往中的问题往往也受到他人的表达、选择和行为影响，这意味着对方同样需要承担相应的责任。如果之后需要和对方进一步沟通，你可以试试：", (("留出表达空间", "表达自己的想法时，也给对方充分表达和回应的空间。"), ("清楚表达需求", "清楚、具体地表达自己的感受和需求。"), ("减少指责表达", "尽量避免使用指责、讽刺或攻击性的表达方式。"))),
...
def generic_round_script(emotion: str, ai_round: int) -> str:
    ...
    lines.append(
        "第一段开头只允许自由生成一句不超过15个汉字的简短、模糊、宽泛的通用衔接套话，随后使用以上第一段脚本；不得通过增加第二句来延长衔接。"
        "衔接句不得表现出已经理解用户的具体处境，不得根据用户输入内容进行调整，也不得加入具体事实、个性化信息、新分析或新建议。"
